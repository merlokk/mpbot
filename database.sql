/******************************************************************************/
/***          Generated by IBExpert 2011.10.01 03.07.2012 19:11:13          ***/
/******************************************************************************/
...
SET GENERATOR GEN_PARAMS_ID TO 170;
...
CREATE PROCEDURE CAN_HELP_FRIEND (
    ROOM_ID INTEGER,
    FRIEND_ID BIGINT)
RETURNS (
    RES SMALLINT)
...
CREATE PROCEDURE GET_TODAY_USED_GIFTS
RETURNS (
    CNT INTEGER)
AS
BEGIN
...
CREATE TABLE EXEC_CONTRACTS (
    ...
    HELP_NAME        VARCHAR(70),
    HELP_SLOTS_LINK  VARCHAR(70),
    HELP_MSG         VARCHAR(1000)
);
...
CREATE TABLE GIFTS_AVAIL (
    ID             BIGINT NOT NULL,
    GAME_ITEMS_ID  INTEGER,
    QTY            INTEGER
);
...
ALTER TABLE FRIENDS_HELP ADD CONSTRAINT PK_FRIENDS_HELP PRIMARY KEY (FRIENDS_ID, ROOM_ID);
...
ALTER TABLE GIFTS_AVAIL ADD CONSTRAINT PK_GIFTS_AVAIL PRIMARY KEY (ID);
...
ALTER TABLE PARAM_GROUP ADD CONSTRAINT PK_PARAM_GROUP PRIMARY KEY (ID);
...
ALTER TABLE FRIENDS_HELP ADD CONSTRAINT FK_FRIENDS_HELP_1 FOREIGN KEY (FRIENDS_ID) REFERENCES FRIENDS (ID);
...
ALTER TABLE PARAMS ADD CONSTRAINT FK_PARAMS_2 FOREIGN KEY (PARAM_GROUP_ID) REFERENCES PARAM_GROUP (ID);
...
CREATE INDEX GIFTS_AVAIL_IDX1 ON GIFTS_AVAIL (GAME_ITEMS_ID);
...
ALTER PROCEDURE CAN_HELP_FRIEND (
    ROOM_ID INTEGER,
    FRIEND_ID BIGINT)
RETURNS (
    RES SMALLINT)
...
begin
  ...
  select FR.ID
  from FRIENDS FR
  left join FRIENDS_HELP FRH on FR.ID = FRH.FRIENDS_ID and FRH.ROOM_ID = :ROOM_ID
  where (FR.ID = :FRIEND_ID) and
        (FR.INGAME = 1) and
        ((FRH.HELP_DT is null) or (FRH.HELP_COUNT > 0) or (dateadd(2 day to FRH.HELP_DT) < current_timestamp))
  into :FRID;

  if (FRID = FRIEND_ID) then
    RES = 1;
...
ALTER PROCEDURE GET_TODAY_USED_GIFTS
RETURNS (
    CNT INTEGER)
AS
begin
  select count(1) CNT
  from GIFTS_REWARD_ADDED
  where EVENT_DATE between current_date and dateadd(1 day to current_date)
  into :CNT;
